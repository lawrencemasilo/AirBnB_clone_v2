...
class TestAmenityDocs(unittest.TestCase):
    ...
    def test_pep8_compliance_amenity(self):
        """Test that models/amenity.py complies with PEP8."""
        style = pep8.StyleGuide(quiet=True)
        outcome = style.check_files(['models/amenity.py'])
        self.assertEqual(outcome.total_errors, 0,
                         "Found code style errors (and warnings).")

    def test_pep8_compliance_test_amenity(self):
        """
        Test that tests/test_models/test_amenity.py complies with PEP8.
        """
        style = pep8.StyleGuide(quiet=True)
